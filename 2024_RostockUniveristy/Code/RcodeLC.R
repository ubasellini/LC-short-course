...
##  Lee-Carter method
##
##  Code prepared for the short guest lecture for the 
##  "Mortality Analysis" course taught by Prof. Roland Rau
...
## no packages can be loaded at Uni Rostock, hence
## loading some useful functions exported from forecast package
## to fit and forecast an ARIMA model
## (available on CRAN and at https://github.com/robjhyndman/forecast)
source("forecast.R")
...
par(mar=c(3,3,1,1))
matplot(a,LMX,t="l",col=hcl.colors(n1),lty=1,xlab="",ylab="")
mtext("ages",side=1,line=1.5,cex=1.5);mtext("log-mortality",side=2,line=2,cex=1.5)
rany <- range(LMX,finite=T)
for(i in 1:n1){
  plot(a, LMX[,i], col=hcl.colors(n1)[i], pch=16, ylim=rany, main=t1[i],xlab="",ylab="")
  mtext("ages",side=1,line=1.5,cex=1.5);mtext("log-mortality",side=2,line=2,cex=1.5)
  Sys.sleep(0.1)
}
...
mtext("years",side=1,line=1.75,cex=1.5);mtext("life exp",side=2,line=2,cex=1.5)
...
## plotting the matrix
image(t1,a,t(LMXres),axes="F")
axis(1);axis(2,las=2);box()
mtext("ages",side=2,line=2,cex=1.5);mtext("year",side=1,line=2,cex=1.5)
dev.off()
...
## plotting
matplot(a,ETAlc,t="l",lty=1,lwd=0.8,col=hcl.colors(n1),xlab="",ylab="")
mtext("ages",side=1,line=1.5,cex = 1.5);mtext("log-mortality",side=2,line=2,cex = 1.5)

rany <- range(LMX,finite=T)
i <- 1
for(i in 1:n1){
  plot(a, LMX[,i], col=hcl.colors(n1)[i], pch=1, ylim=rany, main=t1[i],xlab="",ylab="")
  mtext("ages",side=1,line=1.5,cex=1.5);mtext("log-mortality",side=2,line=2,cex=1.5)
  lines(a,ETAlc[,i],col=hcl.colors(n1)[i],lwd=2)
  Sys.sleep(0.1)
}
...
mtext("years",side=1,line=1.5,cex=1.5);mtext("life exp",side=2,line=2,cex=1.5)
...
## Kappa forecasts
predK <- forecast.Arima(modK,h=nF)
...
for(i in 1:nS){
  ...
  kappa.sim <- simulate.Arima(modK, nsim=nF,
...
mtext("log-mortality",side=2,las=3,line=1.8,cex=1.5)
...
mtext("year",side=1,line=1.8,cex=1.5)
mtext(expression(e[0]),side=2,las=1,line=1.9,cex=1.5)
